...
public abstract class Identified {
	...
	public Identified(URI identity) {
		...
		// TODO: Check if identity is compliant with the "good form".
		String[] splitted_identity = identity.toString().split("/");
		// authority = splitted_identity[0], ID = splitted_identity[1],
		// majorVersion = splitted_identity[2], minorVersion =
		// splitted_identity[3]
		setPersistentIdentity(URI.create(splitted_identity[0] + '/'
				+ splitted_identity[1]));
		setVersion(splitted_identity[2]);
	}
	...
	/**
	 * Sets field variable <code>persistentIdentity</code> to the specified
	 * element.
	 * 
	...
	/**
	 * Set optional field variable <code>persistentIdentity</code> to
	 * <code>null</code>.
	 */
	...
	/**
	 * Calls the Annotation constructor to create a new instance using the
	 * specified parameters, then adds to the list of Annotation instances owned
	 * by this component.
	 * 
	 * @param relation
	 * @param literal
	...
	// * @param value
	// * @deprecated As of release 2.0, URI can only be set when an Identified
	// instance is created.
	// */
